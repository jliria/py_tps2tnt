...
class py_tps2tntApp:
    ...
    def create_widgets(self):
        ...
        self.run_gpa_button.grid(row=5, column=0, padx=5, pady=5)
        ttk.Label(main_frame, text="Perform Generalized Procrustes Analysis.").grid(row=5, column=1, padx=5, pady=5, sticky=tk.W)

        # Show Results
        self.show_results_button = ttk.Button(main_frame, text="Show Results", command=self.show_results, state='disabled')
        self.show_results_button.grid(row=6, column=0, padx=5, pady=5)
        ttk.Label(main_frame, text="Show the aligned landmarks.").grid(row=6, column=1, padx=5, pady=5, sticky=tk.W)

        # Save Results
        self.save_results_button = ttk.Button(main_frame, text="Save Results", command=self.save_results, state='disabled')
        self.save_results_button.grid(row=7, column=0, padx=5, pady=5)
        ttk.Label(main_frame, text="Save the results to files.").grid(row=7, column=1, padx=5, pady=5, sticky=tk.W)
        ...
        self.select_landmarks_button.grid(row=8, column=0, padx=5, pady=5)
        ttk.Label(main_frame, text="Select specific landmarks for EDMA.").grid(row=8, column=1, padx=5, pady=5, sticky=tk.W)

        # Export TNT File
        self.export_tnt_button = ttk.Button(main_frame, text="Export TNT File", command=self.export_tnt_file, state='disabled')
        self.export_tnt_button.grid(row=9, column=0, padx=5, pady=5)
        ttk.Label(main_frame, text="Export the results to a TNT file.").grid(row=9, column=1, padx=5, pady=5, sticky=tk.W)

        # Status Message
        self.status_message = ttk.Label(main_frame, text="Please load a TPS file to start.")
        self.status_message.grid(row=10, column=0, columnspan=2, padx=5, pady=5, sticky=(tk.W, tk.E))
    ...
    def get_species_counts(self):
        ...
        for species in species_list:
            count = simpledialog.askinteger("Input", f"Enter number of specimens for species {species}:")
    ...
    def export_tnt_file(self):
        ...
        if include_mean_coords or include_gpa_coords:
            num_characters += 1  # Configurations will be treat as one character
        ...
        with open(save_tnt_path, 'w') as file:
            ...
            # 1. Export CSize only if is was selected
            if include_centroid_size:
                file.write("&[cont]\n")
